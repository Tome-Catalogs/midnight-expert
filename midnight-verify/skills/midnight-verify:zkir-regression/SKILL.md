...
1. Load the `midnight-verify:verify-correctness` skill via Tome's `get_skill` MCP tool (plugin `midnight-verify`, name `verify-correctness`) — not the `Skill` tool, since this is a Tome-catalog skill — to classify the claim domain
