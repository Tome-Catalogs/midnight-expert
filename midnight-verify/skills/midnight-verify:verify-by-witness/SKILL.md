...
- If the claim would benefit from a full deploy+call lifecycle test, state: "**Recommend devnet E2E:** The orchestrator should dispatch `midnight-verify:sdk-tester` with the compiled contract and witness for full lifecycle verification."
- If local verification (phases 1-4) is sufficient for the claim, state: "**Devnet E2E not required** for this claim."

The orchestrator will decide whether to dispatch `midnight-verify:sdk-tester` based on your recommendation and devnet availability.
...
If the orchestrator indicated PLONK verification is needed, include the build output path in the report so the orchestrator can pass it to `midnight-verify:zkir-checker`:
...
Do NOT remove the base workspace — it's shared across jobs. If the orchestrator needs the build output for `midnight-verify:zkir-checker`, do NOT clean up until the orchestrator confirms the zkir-checker is done.
