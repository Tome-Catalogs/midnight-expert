...
You may load the `midnight-tooling:compact-cli` skill via Tome's `get_skill` MCP tool (not the `Skill` tool, since this skill lives in a Tome catalog) to understand what flags exist and how the CLI works. This is a **hint only** — the CLI output is your evidence, not the skill content.
...
Invoke the `midnight-tooling:install-cli` command via its Tome MCP prompt (not the `Skill` tool — it is a command, not a skill) for installation instructions and retry.
```
