...
Load any of these via Tome's `get_skill` MCP tool (not the `Skill` tool, since these skills live in a Tome catalog) if they would help you write a better test. Do not load them all — only what's relevant to the claim.
...
Or load the `midnight-tooling:compact-cli` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) for details on version management.
...
Load the `midnight-tooling:compact-cli` skill via Tome's `get_skill` MCP tool (not the `Skill` tool, since this skill lives in a Tome catalog) for compilation flags, version management, and troubleshooting.
...
If found, note the path in your report so the orchestrator can dispatch `midnight-verify:zkir-checker` if needed. Do NOT run the checker yourself — your job is compilation and JS runtime execution.
