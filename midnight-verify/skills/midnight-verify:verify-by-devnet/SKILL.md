...
Load the `midnight-tooling:devnet` skill via Tome's `get_skill` MCP tool (not the `Skill` tool, since this skill lives in a Tome catalog) for endpoint URLs and health check patterns. Check that all three services are reachable:
...
- Message: "Devnet not available. Load the `midnight-tooling:devnet` skill via Tome's `get_skill` MCP tool (not the `Skill` tool, since this skill lives in a Tome catalog) for instructions on starting the devnet, then retry."
...
// 2. Set up providers (load the `midnight-tooling:devnet` skill via Tome's `get_skill` MCP tool, not the `Skill` tool, for URLs)
const providers = {
...
2. **Write and compile a minimal contract** using `compact compile --skip-zk` — load the `midnight-tooling:compact-cli` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) for compilation details
