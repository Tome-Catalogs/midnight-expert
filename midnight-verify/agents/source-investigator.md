...
Load the `midnight-verify:verify-by-source` skill via Tome's `get_skill` MCP tool (not the `Skill` tool, as this skill lives in a Tome catalog) and follow it step by step. It tells you exactly how to:
...
**When the claim domain is wallet SDK**, load the `midnight-verify:verify-by-wallet-source` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) instead of the `midnight-verify:verify-by-source` skill. The wallet source skill provides wallet-specific repo routing, package hierarchy context, and strict evidence rules. The general verify-by-source skill is for Compact compiler, ledger, and DApp SDK source — not wallet SDK.

**When the claim domain is ledger/protocol**, load the `midnight-verify:verify-by-ledger-source` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) instead of the `midnight-verify:verify-by-source` skill. The ledger source skill provides Rust crate-level routing across the 24-crate workspace, dependency graph context, and guidance on tracing WASM bindings back to Rust implementations. The general verify-by-source skill is for Compact compiler and DApp SDK source — not ledger internals.
