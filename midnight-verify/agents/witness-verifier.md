...
Load the `midnight-verify:verify-by-witness` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) and follow it step by step. The skill defines a 5-phase pipeline:
...
- For claims that also need PLONK verification, compile without `--skip-zk` and report the build output path so the orchestrator can pass it to `midnight-verify:zkir-checker`.
- You may load the `compact-core:compact-witness-ts` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) as a hint for understanding witness patterns, but your verification results are the evidence, not skill content.
