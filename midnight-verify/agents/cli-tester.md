...
Load the `midnight-verify:verify-by-cli-execution` skill via Tome's `get_skill` MCP tool (not the `Skill` tool, since this skill lives in a Tome catalog) and follow it step by step. It tells you exactly how to:
...
You may load the `midnight-tooling:compact-cli` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) as a hint for understanding CLI flags, compilation patterns, and version management. But the CLI output is your evidence, not the skill content.
