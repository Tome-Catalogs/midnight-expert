...
- **Checker claims** (ZK proof validity, constraint behavior, transcript integrity, proof data correctness) → load the `midnight-verify:verify-by-zkir-checker` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) and follow it step by step
- **Inspection claims** (compiled circuit structure, instruction counts, opcode usage, transcript encoding) → load the `midnight-verify:verify-by-zkir-inspection` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) and follow it step by step
