...
Load the `midnight-verify:verify-by-devnet` skill via Tome's `get_skill` MCP tool (not the `Skill` tool, as this skill lives in a Tome catalog) and follow it step by step. It tells you exactly how to:
