...
```
The D-parameter is the **`(num_permissioned, num_registered)` tuple** — here `(10, 0)`, a fully federated committee. It is NOT a `0.0`–`1.0` scalar. Load the `midnight-node:node-governance` skill via Tome's `get_skill` MCP tool (plugin `midnight-node`, name `node-governance`) — do not use the `Skill` tool, as this skill lives in a Tome catalog.
...
```
`systemParameters_getAriadneParameters` takes a required `epoch_number` (mainchain epoch) and an optional `d_parameter_at` block hash. It returns the D-parameter plus the candidate pools used by Ariadne selection. Load the `midnight-node:node-validator` skill via Tome's `get_skill` MCP tool (plugin `midnight-node`, name `node-validator`) — do not use the `Skill` tool, as this skill lives in a Tome catalog.
