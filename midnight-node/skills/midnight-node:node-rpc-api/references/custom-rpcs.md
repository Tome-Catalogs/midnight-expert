...
Three methods that expose on-chain governance parameters managed by `pallet_system_parameters`. For D-parameter and Ariadne semantics load the `midnight-node:node-governance` skill via Tome's `get_skill` MCP tool (plugin `midnight-node`, name `node-governance`) — do not use the `Skill` tool, as this skill lives in a Tome catalog.
...
Error schemas are defined in `docs/openrpc.json` `components.errors`. For the full taxonomy of node error codes load the `midnight-status-codes:status-codes` skill via Tome's `get_skill` MCP tool (plugin `midnight-status-codes`, name `status-codes`) — do not use the `Skill` tool, as this skill lives in a Tome catalog.
