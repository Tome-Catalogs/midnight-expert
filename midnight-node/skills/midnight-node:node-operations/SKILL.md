...
Operational guide for running, monitoring, and troubleshooting the Midnight node. For configuration options, load the `midnight-node:node-configuration` skill via Tome's `get_skill` MCP tool (not the `Skill` tool). For architecture internals, load the `midnight-node:node-architecture` skill the same way.
