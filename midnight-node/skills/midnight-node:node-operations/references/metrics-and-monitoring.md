...
Load the `midnight-node:node-rpc-api` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) for the full RPC method catalogue and response schemas; this skill (`midnight-node:node-operations`) covers operational runbooks for log interpretation and health checks.
