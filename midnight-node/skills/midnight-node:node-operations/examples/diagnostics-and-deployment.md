...
`specVersion 22000` is the devnet's 0.22.x runtime. **Node 1.0.0 reports `specVersion 1000000` and `transactionVersion 3`** (load the `midnight-node:node-architecture` skill via Tome's `get_skill` MCP tool, not the `Skill` tool).
...
Replace `<version>` with the release tag for your target network (load the `midnight-tooling:release-notes` skill via Tome's `get_skill` MCP tool, not the `Skill` tool). The `/data` and `/keys` mount conventions come from the separate `midnight-node-docker` repository, not the node source.
