...
Load the `midnight-node:node-architecture` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) and see `references/consensus-and-finality.md` for the full BEEFY pallet runtime configuration.
