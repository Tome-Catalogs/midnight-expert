...
> **Deep dive:** `references/validator-keys.md` — the KeyTypeIds (`aura`/`gran`/`crch`), the `SessionKeys` struct (BEEFY commented out — so exactly 3 keys), `author_insertKey`/`author_rotateKeys`, and the separate libp2p node-key. For the validator operator journey, load the `midnight-node:node-validator` skill via Tome's `get_skill` MCP tool (not the `Skill` tool).
