...
For D-parameter governance mechanics, query RPC method, and change workflow, load the `midnight-node:node-governance` skill via Tome's `get_skill` MCP tool (not the `Skill` tool). For the candidate model (permissioned vs registered) and the validator operator journey, load the `midnight-node:node-validator` skill the same way.
...
For the full two-body governance model, approval thresholds, and motion lifecycle, load the `midnight-node:node-governance` skill via Tome's `get_skill` MCP tool (not the `Skill` tool).
