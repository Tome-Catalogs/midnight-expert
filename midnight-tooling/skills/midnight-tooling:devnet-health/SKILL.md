...
Other skills, commands, and scripts that want to invoke these checks should resolve the absolute path from the plugin directory that Tome exposes as `${TOME_PLUGIN_DIR}` (substituted into the entry body when it is served). These scripts live in this skill's directory, so from another entry in the `midnight-tooling` plugin:

```bash
SCRIPTS_ROOT="${TOME_PLUGIN_DIR}/skills/midnight-tooling:devnet-health/scripts"

bash "$SCRIPTS_ROOT/status.sh" --json
bash "$SCRIPTS_ROOT/health.sh" --json
```

A consumer that is itself an entry in this skill can reference its own scripts as `${TOME_SKILL_DIR}/scripts` instead. This keeps the scripts callable regardless of where the plugin is installed.

## When To Use Which Skill

| Question | Skill |
|----------|-------|
| Start, stop, or configure the devnet, generate compose files, or pick versions | `midnight-tooling:devnet` — load via Tome's `get_skill` MCP tool, not the `Skill` tool |
| Run a quick "is it up and serving?" probe from inside an agent or script | this skill |
| Diagnose a broken devnet, port conflicts, indexer sync issues, partial startup | `midnight-tooling:troubleshooting` — load via Tome's `get_skill` MCP tool, not the `Skill` tool (with `references/devnet-issues.md`) |
