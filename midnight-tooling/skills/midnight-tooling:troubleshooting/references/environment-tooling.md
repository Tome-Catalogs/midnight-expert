...
The `midnight-tooling:compact-cli` skill's `references/custom-directories.md` has additional detail on per-project toolchain directory setup across all supported tools — load the skill via Tome's `get_skill` MCP tool, not the `Skill` tool, since it lives in a Tome catalog.
