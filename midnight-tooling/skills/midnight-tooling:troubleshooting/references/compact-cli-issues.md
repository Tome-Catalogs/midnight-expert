...
3. The `midnight-tooling:compact-cli` skill has additional reference material on installation in its `references/installation.md` — load it via Tome's `get_skill` MCP tool, not the `Skill` tool, since this skill lives in a Tome catalog
