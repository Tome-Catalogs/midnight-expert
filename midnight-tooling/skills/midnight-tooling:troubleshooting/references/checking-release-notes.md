...
The `midnight-tooling:release-notes` skill provides comprehensive access to all Midnight component release notes. Load it via Tome's `get_skill` MCP tool (not the `Skill` tool, since this skill lives in a Tome catalog) to fetch notes for specific components and versions.
...
1. **Identify the component directory** using the `midnight-tooling:release-notes` skill's component map (load the skill via Tome's `get_skill` MCP tool, not the `Skill` tool)
