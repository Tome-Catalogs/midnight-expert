...
- Check all services: invoke the `midnight-tooling:devnet` command via its Tome MCP prompt (not the `Skill` tool, not a slash command) with `status` and with `health`
- Check logs for the specific service: invoke the `midnight-tooling:devnet` command via its Tome MCP prompt with `logs --service proof-server`
