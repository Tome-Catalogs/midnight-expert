...
| Quick "is the devnet up and serving?" check, container/HTTP probes | Load the `midnight-tooling:devnet-health` skill via Tome's `get_skill` MCP tool (not the `Skill` tool — this skill lives in a Tome catalog) |
...
- **`references/checking-release-notes.md`** depends on the `midnight-tooling:release-notes` skill and its `references/component-map.md` for component directory mappings — load it via Tome's `get_skill` MCP tool, not the `Skill` tool.
- **`references/compact-cli-issues.md`** references the `midnight-tooling:compact-cli` skill's `references/installation.md` for additional installation detail — load it via Tome's `get_skill` MCP tool, not the `Skill` tool.
- **`references/environment-tooling.md`** references the `midnight-tooling:compact-cli` skill's `references/custom-directories.md` for per-project toolchain setup — load it via Tome's `get_skill` MCP tool, not the `Skill` tool.
- **`references/devnet-issues.md`** depends on the `midnight-tooling:devnet` skill and its references for network lifecycle details — load it via Tome's `get_skill` MCP tool, not the `Skill` tool.
- **`references/proof-server-issues.md`** references the `midnight-tooling:proof-server` skill for API endpoint details, Docker setup, and version management — load it via Tome's `get_skill` MCP tool, not the `Skill` tool.
