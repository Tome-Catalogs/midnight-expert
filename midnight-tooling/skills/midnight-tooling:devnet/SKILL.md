...
The devnet is a local 3-service blockchain network for Midnight development. It runs via Docker Compose and is managed through the `midnight-tooling:devnet` command — invoke it via its Tome MCP prompt, not as a slash command — which uses bash and Docker Compose commands directly.
...
_Invoke `midnight-tooling:devnet` via its Tome MCP prompt with the subcommand shown — not as a slash command._
...
| Network starts but indexer not responding | Indexer still syncing with node | Wait 10-20 seconds after start; invoke `midnight-tooling:devnet health` (Tome MCP prompt) to check readiness |
| Stale chain state causing errors | Corrupted or outdated volumes | Restart with `--remove-volumes` for a clean slate |
| Container name conflicts | Previous containers not cleaned up | Stop the devnet first (invoke `midnight-tooling:devnet stop` via Tome MCP prompt), then start again |
...
| Compose file stale warning | File older than 5 days | Invoke `midnight-tooling:devnet update` (Tome MCP prompt) to refresh image versions |
...
For status and health checks from inside a script, the statusline, or a doctor agent, load the `midnight-tooling:devnet-health` skill via Tome's `get_skill` MCP tool (plugin `midnight-tooling`, name `devnet-health`) — do not use the `Skill` tool, as this skill lives in a Tome catalog. It provides two bash scripts (`status.sh`, `health.sh`) that report container state and per-service HTTP health using only `bash`, `docker`, and `curl`.
