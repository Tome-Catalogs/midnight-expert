...
Funding test wallets on the local devnet. Load the
`midnight-wallet:managing-test-wallets` skill via Tome's `get_skill` MCP tool
(plugin `midnight-wallet`, name `managing-test-wallets`) — do not use the
`Skill` tool, as this skill lives in a Tome catalog — for the SDK-driven
funding pattern that uses this seed.
