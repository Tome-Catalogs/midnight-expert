...
> Then run the following bash commands and report results. Do NOT output anything except lines in the exact format `CHECK_NAME | STATUS | DETAIL`. Do not include markdown fences or any other text.
>
> Commands to run:
> 1. `PLUGIN_ROOT="${TOME_PLUGIN_DIR}"`
...
>    - `PLUGIN_ROOT="${TOME_PLUGIN_DIR}"`
>    - `DEVNET_SCRIPTS="$PLUGIN_ROOT/skills/midnight-tooling:devnet-health/scripts"`
>
...
> Then run the following bash commands. ONLY output lines for checks that FAIL. If a check passes, output NOTHING for it. Use the exact format `CHECK_NAME | STATUS | DETAIL`. Do not include markdown fences or any other text.
>
> Commands to run:
> 1. `PLUGIN_ROOT="${TOME_PLUGIN_DIR}"`
> 2. `SCRIPTS_ROOT="$PLUGIN_ROOT/scripts/doctor"`
...
| COMPACT_DIRECTORY not set but .compact/ exists | Invoke the `midnight-tooling:install-cli` command (Tome MCP prompt, not a slash command) with `install for this project` |
...
| Proof server not running | Invoke `midnight-tooling:devnet start` (Tome MCP prompt, not a slash command) to start the devnet (includes node, indexer, and proof server) |
| Node not running | Invoke `midnight-tooling:devnet start` (Tome MCP prompt, not a slash command) to start the devnet |
| Indexer not running | Invoke `midnight-tooling:devnet start` (Tome MCP prompt, not a slash command) to start the devnet |
| Stale devnet containers | Invoke `midnight-tooling:devnet stop --remove-volumes` then `midnight-tooling:devnet start` (Tome MCP prompt, not a slash command) |
...
| octocode MCP server not connected | Verify the midnight-tooling plugin is enabled (`tome plugin list`), check that Node.js and npx are available, and restart your agent / MCP client to reconnect MCP servers |
