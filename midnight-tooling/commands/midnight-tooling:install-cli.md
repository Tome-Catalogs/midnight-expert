...
**Claude Code settings** (offer only when the user works in Claude Code):

`.claude/settings.json` is read only by Claude Code — other harnesses ignore it, so skip this option unless the user is on Claude Code; the direnv and mise options above are the harness-neutral choices. When it applies, check if `.claude/settings.json` exists and already has `COMPACT_DIRECTORY` in the `env` field. If not, create or update the file:
