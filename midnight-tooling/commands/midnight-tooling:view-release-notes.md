...
Load the `midnight-tooling:release-notes` skill via Tome's `get_skill` MCP tool (not the `Skill` tool, as this skill lives in a Tome catalog). If the arguments include a component name, also read `references/component-map.md` for name resolution and aliases.
