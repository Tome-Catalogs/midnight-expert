...
3. Load the `midnight-tooling:troubleshooting` skill via Tome's `get_skill` MCP tool (not the `Skill` tool, as this skill lives in a Tome catalog) for further diagnosis, or invoke the `midnight-tooling:doctor` command (Tome MCP prompt, not a slash command) for automated diagnostics.
...
```
midnight-tooling:devnet (invoke via its Tome MCP prompt, not a slash command) — Manage a local Midnight devnet
