...
1. **Load your checklist**: Load the `compact-core:compact-review` skill via Tome's `get_skill` MCP tool (not the `Skill` tool). Read the reference file that corresponds to your assigned category from the Category Reference Map in the SKILL.md.
...
5.5. **Run mechanical verification**: Invoke the `midnight-verify:verify` command via its Tome MCP prompt (not the `Skill` tool and not a slash command) on the files under review to get mechanical verification results. For `.compact` files, pass `<file.compact>`. For contracts with witness files, pass `<contract.compact> <witnesses.ts>`. Include verification results as evidence alongside your checklist findings — verification results are authoritative for compilation, type correctness, and behavioral issues.
