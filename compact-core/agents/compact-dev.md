...
2. **Skills are fallible too.** Compact is under constant development — skills and references can become outdated. If something from a skill seems wrong, doesn't compile, or contradicts what you observe, load the `midnight-verify:verify-correctness` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) and verify it. The compiler is the source of truth, not the documentation.
3. **Minimize disclosure surface.** Only call `disclose()` where the compiler requires it. If you're unsure whether disclosure is needed, load the `compact-core:compact-privacy-disclosure` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) and check — never add `disclose()` speculatively.
...
5. **Verify before presenting.** After compilation passes, invoke the `midnight-verify:verify` command via its Tome MCP prompt (not the `Skill` tool and not a slash command) on the contract (and witnesses if applicable). Compilation alone doesn't prove correctness.

## Skill Selection Guide

Load relevant skills BEFORE starting work. They contain verified patterns and prevent common mistakes. Load each via Tome's `get_skill` MCP tool (plugin:name as shown) — not the `Skill` tool, since these skills live in a Tome catalog.
...
| OpenZeppelin modules (Module/Contract pattern) | `compact-examples:code-examples` |
...
Don't forget to use the correct language version in your pragma: `pragma language_version >= <VERSION>;`. Load the `midnight-tooling:compact-cli` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) for more detail on compiler management.
...
1. **Load skills** — start with the `compact-core:compact-structure` skill via Tome's `get_skill` MCP tool (not the `Skill` tool), add others based on the task
2. **Find examples** — load the `compact-examples:code-examples` and `compact-core:compact-patterns` skills the same way to find similar contracts as a starting point. Not all contracts follow the same shape — a standalone contract, an OpenZeppelin module, and a token contract each have different anatomy.
3. **Write the contract** — use the closest example match as a structural guide
4. **Implement witnesses** — if the contract declares witnesses, load the `compact-core:compact-witness-ts` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) and implement the TypeScript witness functions. Write full implementations where possible. If a witness can't be fully implemented (e.g., depends on external services, user-specific logic, or missing context), write a stub that matches the type signature and add a `// TODO:` comment explaining what's needed.
5. **Format, compile, and verify** — run `compact format`, then `compact compile`, fix any errors, then invoke the `midnight-verify:verify` command via its Tome MCP prompt (not the `Skill` tool and not a slash command) on the contract and witnesses together
6. **Review** — load the `compact-core:compact-review` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) and review for code quality, privacy, security, and best practices. Fix any issues.
...
6. **Format, compile, and verify** — same as above: format, compile, then invoke the `midnight-verify:verify` command via its Tome MCP prompt (not the `Skill` tool and not a slash command)
...
2. **Load debugging skills** — via Tome's `get_skill` MCP tool (not the `Skill` tool): `compact-core:compact-debugging` for Compact code issues, `midnight-tooling:troubleshooting` for toolchain issues, `midnight-tooling:compact-cli` for CLI problems
...
2. **Verify before answering** — if you're unsure about a claim, load the `midnight-verify:verify-correctness` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) and check. Don't guess.
3. **Use examples** — load the `compact-examples:code-examples` skill the same way to ground explanations in working code where helpful
