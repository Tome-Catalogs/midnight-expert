...
This skill covers the TypeScript half of every Compact contract: implementing witnesses, understanding compiler-generated types, and using the Contract runtime. For Compact witness declarations and disclosure rules, load the `compact-core:compact-structure` skill; for privacy patterns using witnesses, load the `compact-core:compact-privacy-disclosure` skill; for standard library functions referenced in witnesses, load the `compact-core:compact-standard-library` skill — all via Tome's `get_skill` MCP tool, not the `Skill` tool, since these are Tome-catalog skills.
...
Witness implementations can be mechanically verified against their contract declarations by invoking the `midnight-verify:verify` command via its Tome MCP prompt (not the `Skill` tool, and not as a slash command) with the contract and witness file paths, e.g. `<contract.compact> <witnesses.ts>`.
...
Strongly recommend invoking the `midnight-verify:verify` command (Tome MCP prompt) after writing or modifying any witness implementation. Do not consider a witness implementation complete until verification passes.
