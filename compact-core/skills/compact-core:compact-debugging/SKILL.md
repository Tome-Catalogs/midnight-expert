...
**If compatibility:** fetch live support matrix from `midnightntwrk/midnight-docs` (`docs/relnotes/support-matrix.mdx`), compare against installed versions. Recommend invoking the `midnight-tooling:doctor` command via its Tome MCP prompt (not the `Skill` tool) for automated diagnostics.
...
Before any investigation, invoke the `midnight-verify:verify` command via its Tome MCP prompt (not the `Skill` tool) on the contract:
...
If the `midnight-verify:verify` command (invoked via its Tome MCP prompt) identifies the issue directly, present the finding — no further investigation may be needed.

## Symptom-Driven Triage Table

Skills named in the "Route To Skill" column are Tome catalog skills — load each via Tome's `get_skill` MCP tool (plugin `compact-core`, name as listed below), not the `Skill` tool.
...
**Skills (loaded on demand via triage routing, through Tome's `get_skill` MCP tool — not the `Skill` tool, since these are catalog skills; plugin `compact-core` unless noted):**
...
- `midnight-tooling:doctor` — invoke via its Tome MCP prompt (not the `Skill` tool); automated version diagnostics, environment health
