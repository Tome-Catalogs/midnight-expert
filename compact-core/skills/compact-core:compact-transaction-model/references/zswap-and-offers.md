...
For full details on token issuance, minting, and burning from Compact contracts, load the `compact-core:compact-tokens` skill via Tome's `get_skill` MCP tool (not the `Skill` tool, since this skill lives in a Tome catalog).
