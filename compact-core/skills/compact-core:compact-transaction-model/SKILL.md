...
This skill covers how Compact circuits map to on-chain transaction execution: the three-stage lifecycle (well-formedness, guaranteed, fallible), `kernel.checkpoint()` placement, transaction composition, merging for atomic swaps, the state model, and the fee/gas system. For contract anatomy and circuit/witness design, load the `compact-core:compact-structure` skill; for ledger ADT types and state design, load the `compact-core:compact-ledger` skill; for token minting, sending, and receiving, load the `compact-core:compact-tokens` skill — all via Tome's `get_skill` MCP tool, not the `Skill` tool, since these are Tome-catalog skills.
