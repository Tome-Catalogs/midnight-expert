...
- **Compact CLI** is installed (`compact --version`) — if not, invoke the `midnight-tooling:install-cli` command via its Tome MCP prompt (not the `Skill` tool)

Invoke the `midnight-tooling:doctor` command via its Tome MCP prompt (not the `Skill` tool) to check all prerequisites at once.
...
**Your first stop for any error code or message is the `midnight-status-codes:lookup` command — invoke it via its Tome MCP prompt (not the `Skill` tool).**
...
wraps the real `Custom(N)` cause). For environment problems, invoke the
`midnight-tooling:doctor` command via its Tome MCP prompt (not the `Skill` tool).
...
Stop the devnet if you no longer need it — invoke the `midnight-tooling:devnet` command via its Tome MCP prompt (not the `Skill` tool) with `stop --remove-volumes`:
