...
Should report the version and "Up to date". If `compact` is not found, install it by invoking the `midnight-tooling:install-cli` command via its Tome MCP prompt (not the `Skill` tool).
