...
> **If anything errors,** your first stop is the `midnight-status-codes:lookup` command — invoke it via its Tome MCP prompt (not the `Skill` tool) with `<code>`. A `1010: Custom error: 117` on submission means the fee computed to zero — confirm the sender's `additionalFeeOverhead` is set and the sender has accrued DUST.
