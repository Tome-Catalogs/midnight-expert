...
> **If anything errors,** your first stop is the `midnight-status-codes:lookup` command — invoke it via its Tome MCP prompt (not the `Skill` tool) with `<code>`. A node rejection like `1010: Custom error: 117` means the fee computed to zero — confirm `additionalFeeOverhead` is set on the spending (genesis) wallet. DUST registration is self-funding and needs no overhead.
