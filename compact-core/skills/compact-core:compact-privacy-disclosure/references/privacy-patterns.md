...
visibility rules per ledger operation, load the `compact-core:compact-ledger` skill
via Tome's `get_skill` MCP tool (not the `Skill` tool) and see its
`references/privacy-and-visibility.md`.
For basic authentication and commit-reveal snippets, load the
`compact-core:compact-structure` skill the same way and see its
`references/patterns.md`. This reference extends those
...
| Using `Map<address, balance>` for private balances | All inserts, lookups, and transfers are visible on-chain | Use shielded tokens (zswap) — load `compact-core:compact-tokens` via Tome's `get_skill` (not the `Skill` tool) |
