...
This skill covers Compact language mechanics: the type system, operators, expressions, type casting, control flow, the module system, and the standard library. It does not cover contract architecture (ledger declarations, circuit design, witness integration, constructor layout) -- for that, load the `compact-core:compact-structure` skill via Tome's `get_skill` MCP tool (not the `Skill` tool). Use this skill when you need to know how the language works rather than how to structure a contract.
