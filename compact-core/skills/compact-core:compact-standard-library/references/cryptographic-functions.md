...
For full documentation on hashing and commitment functions, including disclosure rules, domain separation patterns, and worked code examples, load the `compact-core:compact-language-ref` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) and see its `references/stdlib-functions.md`. The table below is a quick-reference overview.
