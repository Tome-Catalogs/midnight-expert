...
Alphabetical index of every Compact standard library export. Use this as a quick lookup to find the name, kind, and authoritative documentation location for any stdlib symbol. For detailed usage, examples, and semantics, load the skill named by the authoritative location path via Tome's `get_skill` MCP tool (not the `Skill` tool, since these are Tome-catalog skills) and see the referenced file within it.
...
Authoritative location paths use the format `<skill-name>/references/<file>.md`, referring to reference files within sibling skills under the `compact-core` plugin. Each path points to the skill and file that contains the most detailed documentation for that particular export — load the named skill (as `compact-core:<skill-name>`) via Tome's `get_skill` MCP tool, not the `Skill` tool, then open the referenced file.
