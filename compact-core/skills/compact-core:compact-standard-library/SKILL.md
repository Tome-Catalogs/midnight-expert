...
This is the single authoritative index of everything `import CompactStandardLibrary;` provides. Every type, constructor, circuit, and builtin documented here has been verified against the Compact compiler. For contract anatomy and scaffold patterns, load the `compact-core:compact-structure` skill via Tome's `get_skill` MCP tool (not the `Skill` tool). For language mechanics (types, operators, control flow), load the `compact-core:compact-language-ref` skill the same way. For ledger ADT state design and privacy, load the `compact-core:compact-ledger` skill the same way. For token mint/send/receive operations and patterns, load the `compact-core:compact-tokens` skill the same way. This skill follows a verification-first philosophy: when in doubt, verify -- never assume a function exists.
...
| Dispatch the verify agent | invoke the `midnight-verify:verify` command via its Tome MCP prompt (not the `Skill` tool) with the claim | Runs source-first verification and (optionally) executes a minimal contract end-to-end. |
...
For full documentation with examples, disclosure rules, and persistent vs. transient guidance, load the `compact-core:compact-language-ref` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) and see its `references/stdlib-functions.md`.
...
For deep documentation on each function including examples and disclosure rules, load the `compact-core:compact-language-ref` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) and see its `references/stdlib-functions.md`.
...
> For complete signatures, parameters, nonce management, and merge strategies, load the `compact-core:compact-tokens` skill via Tome's `get_skill` MCP tool (not the `Skill` tool).
...
> For complete ADT operation tables, nested composition, and state design patterns, load the `compact-core:compact-ledger` skill via Tome's `get_skill` MCP tool (not the `Skill` tool).
...
Load any of the following via Tome's `get_skill` MCP tool (plugin `compact-core`, name as shown) — do not use the `Skill` tool, since these skills live in a Tome catalog.
