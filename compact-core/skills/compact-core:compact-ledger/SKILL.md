...
This skill covers everything about on-chain state in Compact: declaring ledger fields, choosing and using ADT types, initializing state in constructors, designing state for privacy, and understanding what is visible on-chain. It does not cover general language mechanics (types, operators, control flow) -- for those, load the `compact-core:compact-language-ref` skill via Tome's `get_skill` MCP tool (not the `Skill` tool). It does not cover overall contract anatomy or circuit/witness design -- for those, load the `compact-core:compact-structure` skill the same way.
