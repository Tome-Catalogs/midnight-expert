...
For complete state machine pattern examples with authentication, load the `compact-core:compact-structure` skill via Tome's `get_skill` MCP tool (not the `Skill` tool, since this skill lives in a Tome catalog) and see its `references/patterns.md`.
