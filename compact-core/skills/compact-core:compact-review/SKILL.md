...
Every review MUST include invoking the `midnight-verify:verify` command via its Tome MCP prompt (not the `Skill` tool, and not as a slash command) on the contract, passing the contract path as the argument:

    <contract.compact>

For contracts with TypeScript witness implementations, pass both the contract and witness paths:
