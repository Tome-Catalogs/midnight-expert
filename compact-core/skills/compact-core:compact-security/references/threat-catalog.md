...
Reference shorthand (SR/TR/PR entries live in the `compact-core:compact-review` skill — load it via Tome's `get_skill` MCP tool, not the `Skill` tool, since it lives in a Tome catalog):
