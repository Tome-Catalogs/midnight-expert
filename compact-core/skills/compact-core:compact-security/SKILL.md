...
Apply the threat model, then pull granular checklist items from these existing references. Read `references/threat-catalog.md` for the full index. Rows pointing at `compact-core:compact-review` require loading that skill via Tome's `get_skill` MCP tool (not the `Skill` tool, since it lives in a Tome catalog).
...
- suggested command: invoke the `midnight-verify:verify` command via its Tome MCP prompt (not the `Skill` tool) with "<claim>" (or with <file.compact> [<witnesses.ts>])
```
