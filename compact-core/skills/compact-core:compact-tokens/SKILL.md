...
This skill covers tokens on Midnight: choosing between shielded and unshielded approaches, using the standard library mint/send/receive functions, understanding token colors and domain separators, and the NIGHT/DUST token model. It does not cover ledger ADT types or state design -- those belong in the `compact-core:compact-ledger` skill, loaded via Tome's `get_skill` MCP tool (not the `Skill` tool, since this skill lives in a Tome catalog). It does not cover overall contract anatomy or circuit/witness design -- those belong in the `compact-core:compact-structure` skill, loaded the same way.
