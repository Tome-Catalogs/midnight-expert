...
**Fixes:**
1. Install the Compact CLI: invoke the `midnight-tooling:install-cli` command via its Tome MCP prompt (not the `Skill` tool)
...
4. Invoke the `midnight-tooling:doctor` command via its Tome MCP prompt (not the `Skill` tool) for detailed diagnostics
...
- Invoke the `midnight-tooling:doctor` command via its Tome MCP prompt (not the `Skill` tool) for comprehensive environment diagnostics
- Load the `midnight-tooling:troubleshooting` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) for error-specific guidance
- Load the `midnight-tooling:compact-cli` skill the same way for CLI-specific issues
- Load the `midnight-tooling:proof-server` skill the same way for Docker and proof server issues
