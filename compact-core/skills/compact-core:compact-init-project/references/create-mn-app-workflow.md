...
Invoke the `midnight-tooling:doctor` command via its Tome MCP prompt (not the `Skill` tool) to verify the development environment is ready.
...
2. For missing Compact CLI: invoke the `midnight-tooling:install-cli` command via its Tome MCP prompt (not the `Skill` tool)
3. For Docker issues: load the `midnight-tooling:proof-server` skill via Tome's `get_skill` MCP tool (not the `Skill` tool)
4. For Node.js issues: user must install Node.js 22+ from https://nodejs.org/
5. Re-invoke the `midnight-tooling:doctor` command (Tome MCP prompt) after fixes to confirm
...
Invoke the `midnight-tooling:devnet` command via its Tome MCP prompt (not the `Skill` tool) with `start`
...
Point the user to these compact-core skills for customizing their contract. Load each via Tome's `get_skill` MCP tool (not the `Skill` tool), plugin `compact-core`:
