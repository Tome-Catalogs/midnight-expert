...
Detailed reference for understanding how ledger type choices affect storage costs, privacy tradeoffs, and overall contract efficiency. For full ADT operation semantics, load the `compact-core:compact-ledger` skill via Tome's `get_skill` MCP tool (not the `Skill` tool). For privacy pattern design, load `compact-core:compact-privacy-disclosure` the same way.
