...
This skill covers the cost model for Compact smart contracts across three dimensions: circuit/proving costs, runtime gas costs, and state costs. For loop and vector syntax details, load the `compact-core:compact-language-ref` skill via Tome's `get_skill` MCP tool — do not use the `Skill` tool. For ADT operation semantics, load `compact-core:compact-ledger` the same way. For hash and commitment function signatures, load `compact-core:compact-standard-library`. For pure circuit declarations, load `compact-core:compact-structure`.
