...
Load the `compact-core:compact-security` skill via Tome's `get_skill` MCP tool (not the `Skill` tool); walk references/threat-catalog.md and
references/witness-trust-boundary.md; pull granular criteria from the `compact-core:compact-review` skill's
...
block for every Critical and High finding; if none, emit (none). Do NOT invoke the `midnight-verify:verify` command
...
For the **target** contract, regardless of requests, invoke the `midnight-verify:verify` command via its Tome MCP prompt (not the `Skill` tool and not a slash command), passing:
...
- `type: poc` or `type: source` → invoke `midnight-verify:verify` (Tome MCP prompt) with `"<claim>"`.
- `type: target` → invoke `midnight-verify:verify` (Tome MCP prompt) with `<contract.compact> [<witnesses.ts>]` (reuse the target result above if identical).
