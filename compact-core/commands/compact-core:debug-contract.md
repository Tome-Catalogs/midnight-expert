...
Before any analysis, invoke the `midnight-verify:verify` command via its Tome MCP prompt (not the `Skill` tool and not a slash command) on the contract to establish a mechanical baseline, passing:
...
Pass the verification result as context to the subsequent analysis steps. If the `midnight-verify:verify` command identifies the issue directly, present the finding to the user — no further investigation may be needed.
...
Load the `compact-core:compact-debugging` skill via Tome's `get_skill` MCP tool (not the `Skill` tool). Pass context based on mode:
