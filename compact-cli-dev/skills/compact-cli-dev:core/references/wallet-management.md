...
> For comprehensive Wallet SDK API reference, load the `midnight-wallet:wallet-sdk` skill via Tome's `get_skill` MCP tool (not the `Skill` tool). This document covers wallet construction patterns specific to the CLI context.
