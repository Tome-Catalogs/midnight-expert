...
1. **`compact-cli-dev:core`** — load via Tome's `get_skill` MCP tool (not the `Skill` tool). CLI patterns, project structure, command reference, template architecture, and reference docs for Oclif patterns, wallet management, provider setup, contract lifecycle, and error handling.
...
Invoke the `compact-cli-dev:init` command via its Tome MCP prompt (not the `Skill` tool, and not as a slash command) to scaffold from the template:

```
compact-cli-dev:init
```

Pass along any relevant context from the user's request (directory, project name, contract name, contract path) as arguments to the prompt. For example:
...
- **Never read template files from the plugin.** Invoke the `compact-cli-dev:init` command via its Tome MCP prompt (not the `Skill` tool, and not as a slash command) to scaffold from the template. The template engine handles variable substitution and file generation.
