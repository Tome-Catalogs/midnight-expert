...
The templates and the renderer both live inside this plugin's installation directory, which Tome exposes as `${TOME_PLUGIN_DIR}` (substituted into this command body when it is invoked):

```bash
PLUGIN_ROOT="${TOME_PLUGIN_DIR}"
TEMPLATE_DIR="${PLUGIN_ROOT}/skills/compact-cli-dev:core/templates/cli"
RENDER_SCRIPT="${PLUGIN_ROOT}/skills/compact-cli-dev:core/scripts/render-template.mjs"
...
Render the template via the vendored, dependency-free renderer (`skills/compact-cli-dev:core/scripts/render-template.mjs`), which reads a JSON job from stdin. It needs only Node — there is nothing to install. Re-resolve `PLUGIN_ROOT` inside the same command so the path is correct even though shell variables do not persist between steps, then pipe the JSON job in. Substitute the literal `TEMPLATE_DIR` from Step 4, the resolved output `<directory>`, and the Step 3 context values:

```bash
PLUGIN_ROOT="${TOME_PLUGIN_DIR}"
echo '{"template":"<TEMPLATE_DIR>","output":"<directory>","context":{"PROJECT_NAME":"...","CLI_PACKAGE_NAME":"...","CONTRACT_NAME":"...","CONTRACT_PACKAGE":"...","CONTRACT_ZK_CONFIG_PATH":"...","GENERATED_AT":"..."}}' | node "$PLUGIN_ROOT/skills/compact-cli-dev:core/scripts/render-template.mjs"
```
