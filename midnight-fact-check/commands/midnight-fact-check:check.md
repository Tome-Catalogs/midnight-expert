...
Check that the `midnight-verify` plugin is available: attempt to load the `midnight-verify:verify-correctness` skill via Tome's `get_skill` MCP tool (plugin `midnight-verify`, name `verify-correctness`) — not the `Skill` tool, as it lives in a Tome catalog.
...
6. Merge all outputs using the vendored merge script in concat mode. The script (`skills/midnight-fact-check:fact-check-extraction/scripts/merge.mjs`) is dependency-free and needs only Node — there is nothing to install. Re-resolve the plugin root in the same command, since shell variables do not persist between steps:
   ```bash
   PLUGIN_ROOT="${TOME_PLUGIN_DIR}"
   node "$PLUGIN_ROOT/skills/midnight-fact-check:fact-check-extraction/scripts/merge.mjs" --mode concat -o "$RUN_DIR/extracted-claims.json" "$RUN_DIR/extracted-chunk-1.json" "$RUN_DIR/extracted-chunk-2.json" ...
   ```
...
5. Merge all copies with the vendored merge script in update mode (re-resolve the plugin root in the same command, since shell variables do not persist between steps):
   ```bash
   PLUGIN_ROOT="${TOME_PLUGIN_DIR}"
   node "$PLUGIN_ROOT/skills/midnight-fact-check:fact-check-extraction/scripts/merge.mjs" --mode update --original "$RUN_DIR/extracted-claims.json" -o "$RUN_DIR/classified-claims.json" "$RUN_DIR/extracted-claims.compact-classifier.json" "$RUN_DIR/extracted-claims.sdk-classifier.json" "$RUN_DIR/extracted-claims.zkir-classifier.json" "$RUN_DIR/extracted-claims.witness-classifier.json"
   ```
...
- You dispatch midnight-verify agents directly to verify claims — there is no separate fact-check verification agent
...
1. Load the `midnight-fact-check:fact-check-reporting` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) for templates.
