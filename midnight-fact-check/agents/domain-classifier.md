...
1. Load the `midnight-fact-check:fact-check-classification` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) — it defines domain boundaries and tagging rules.
