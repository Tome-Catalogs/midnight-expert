...
1. Load the `midnight-fact-check:fact-check-extraction` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) — it defines what a testable claim is and the output schema.
