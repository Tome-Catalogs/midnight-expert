...
- Load the `midnight-dapp-dev:core` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) — Provider architecture, state management,
...
1. Load the `midnight-dapp-dev:init` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) to scaffold the template
...
- Handle contract compilation — defer to the `compact-core:compact-init-project` skill
