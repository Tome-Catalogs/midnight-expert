...
This skill covers the Midnight DApp Connector API for browser-based wallet integration: connecting to the Lace wallet extension, using the ConnectedAPI for transactions and balances, handling errors, and building React 19.x / Next.js 16.x DApps. For Node.js CLI deployment with WalletFacade and HD keys, load the `midnight-dapp-dev:midnight-sdk` skill through Tome (`get_skill`) rather than the `Skill` tool. For contract runtime and witness types, load the `compact-core:compact-witness-ts` skill through Tome (`get_skill`) rather than the `Skill` tool. For local development network setup, load the `midnight-tooling:devnet` skill via Tome's `get_skill` MCP tool (not the `Skill` tool).
