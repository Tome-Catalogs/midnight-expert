...
Complete type definitions for the Midnight DApp Connector API v4.0.1. For usage patterns and React/Next.js integration, load the `midnight-dapp-dev:dapp-connector` skill via Tome's `get_skill` MCP tool (plugin `midnight-dapp-dev`, name `dapp-connector`) — do not use the `Skill` tool, as this skill lives in a Tome catalog. For building SDK providers from these types, see `references/browser-providers.md`.
