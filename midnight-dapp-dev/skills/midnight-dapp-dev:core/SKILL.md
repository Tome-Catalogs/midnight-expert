...
To scaffold a new UI + API package, load the `midnight-dapp-dev:init` skill via Tome's `get_skill` MCP tool (plugin `midnight-dapp-dev`, name `init`) — do not use the `Skill` tool, as this skill lives in a Tome catalog.
