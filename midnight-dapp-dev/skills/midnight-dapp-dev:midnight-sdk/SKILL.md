...
Comprehensive reference for the Midnight.js SDK (v4.1.1 — all `@midnight-ntwrk/midnight-js-*` packages move in lockstep): the core packages, the MidnightProviders architecture, the full transaction lifecycle, advanced contract operations, observable patterns, and testkit-js. For the deployment workflow, see `references/transaction-lifecycle.md`. For TypeScript witness implementation, load the `compact-core:compact-witness-ts` skill via Tome's `get_skill` MCP tool (plugin `compact-core`, name `compact-witness-ts`) — do not use the `Skill` tool, as this skill lives in a Tome catalog. For browser wallet integration via the DApp Connector, load the `midnight-dapp-dev:dapp-connector` skill the same way (via Tome's `get_skill`, not the `Skill` tool).

For the underlying Wallet SDK packages (WalletFacade, HD derivation, three-wallet architecture), load the `midnight-wallet:wallet-sdk` skill via Tome's `get_skill` MCP tool (plugin `midnight-wallet`, name `wallet-sdk`) — do not use the `Skill` tool, as this skill lives in a Tome catalog.
...
- **Browser**: Built from `ConnectedAPI.balanceUnsealedTransaction` (load the `midnight-dapp-dev:dapp-connector` skill via Tome's `get_skill`, not the `Skill` tool)
...
- **Browser**: In-memory `Map` or IndexedDB (load the `midnight-dapp-dev:dapp-connector` skill via Tome's `get_skill`, not the `Skill` tool)
