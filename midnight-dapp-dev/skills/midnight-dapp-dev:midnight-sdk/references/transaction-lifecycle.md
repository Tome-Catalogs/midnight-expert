...
Complete reference for the Midnight transaction lifecycle: from circuit call construction through ZK proving, balancing, submission, and on-chain finalization. For package-level API details, see `references/package-reference.md`. For the high-level deployment workflow, load the `midnight-dapp-dev:midnight-sdk` skill via Tome's `get_skill` MCP tool (plugin `midnight-dapp-dev`, name `midnight-sdk`) — do not use the `Skill` tool, as this skill lives in a Tome catalog.
