...
Use this lookup when you have a specific error code or message and need a quick answer. Load the `midnight-status-codes:status-codes` skill via Tome's `get_skill` MCP tool (plugin `midnight-status-codes`, name `status-codes`) — do not use the `Skill` tool, as this skill lives in a Tome catalog — for its reference files when you need deep context about an error category, want to understand the error hierarchy, or need to browse all errors from a component.
