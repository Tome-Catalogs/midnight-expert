...
For the fastest path, invoke the `midnight-status-codes:lookup` command via its Tome MCP prompt (not the `Skill` tool, not a slash command) with `<code-or-search-term>`, instead of reading reference files.
...
1. Invoke the `midnight-status-codes:lookup` command via its Tome MCP prompt (not the `Skill` tool, not a slash command) with `<error-text-or-code>`.
2. Or search by keyword: invoke `midnight-status-codes:lookup` (Tome MCP prompt) with `--search "<key-phrase>"`.
