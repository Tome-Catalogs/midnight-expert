...
Load the `midnight-status-codes:status-codes-lookup` skill via Tome's `get_skill` MCP tool (not the `Skill` tool, as this skill lives in a Tome catalog) for script location and output format.
...
To get the script path, load the `midnight-status-codes:status-codes-lookup` skill via Tome's `get_skill` MCP tool (not the `Skill` tool, as this skill lives in a Tome catalog) — it documents the `${TOME_SKILL_DIR}/scripts/lookup.sh` path.
...
- Suggest invoking the `midnight-status-codes:lookup` command (Tome MCP prompt, not a slash command) with `--sources` to see available sources
