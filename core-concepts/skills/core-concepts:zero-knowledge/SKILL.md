...
### disclose()
Marks the boundary between private (witness-tainted) and public data in Compact. Required when witness-derived values flow to public context (ledger writes, assert conditions, circuit returns). Commitment functions (`persistentCommit`) clear witness taint; hash functions (`persistentHash`) do not. Load the `compact-core:compact-privacy-disclosure` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) for the full disclosure syntax reference.
