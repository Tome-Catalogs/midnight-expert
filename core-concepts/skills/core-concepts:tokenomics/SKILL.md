...
Because transactions use shielded DUST for fees and transaction details are protected by zero-knowledge proofs, block producers cannot extract value by reordering, front-running, or sandwiching transactions. The shielded nature of DUST payments and transaction contents eliminates the information asymmetry that enables MEV on other networks. Load the `core-concepts:zero-knowledge` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) for details on how ZK proofs protect transaction privacy.
...
Smart contracts interact with tokens through the Compact standard library. Shielded token operations use the Zswap protocol to maintain privacy, while unshielded tokens are handled through direct ledger operations. Load the `compact-core:compact-tokens` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) for detailed token operation syntax and patterns.

Transaction fees paid in DUST are handled at the protocol level. The fee model and gas costs for contract execution are covered by the `compact-core:compact-transaction-model` skill — load it via Tome's `get_skill` MCP tool (not the `Skill` tool) for details.
