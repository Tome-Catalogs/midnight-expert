...
Compact contracts interact with Zswap through standard library functions for receiving, sending, and minting tokens. These are stdlib circuit calls imported via `import CompactStandardLibrary;`. Load the `compact-core:compact-standard-library` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) for function signatures and usage details.
