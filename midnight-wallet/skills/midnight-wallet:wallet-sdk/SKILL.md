...
changes between SDK versions. Load the `midnight-wallet:sdk-regression-check`
skill via Tome's `get_skill` MCP tool (plugin `midnight-wallet`, name
`sdk-regression-check`) — not the `Skill` tool — and run it before
trusting any pattern here.
...
DApp with a browser extension wallet (Lace or other), load the
`midnight-dapp-dev:dapp-connector` skill via Tome's `get_skill` MCP tool
(plugin `midnight-dapp-dev`, name `dapp-connector`) — not the `Skill`
tool — instead.
