...
Before using any pattern from this skill, load the
`midnight-wallet:sdk-regression-check` skill via Tome's `get_skill` MCP
tool (plugin `midnight-wallet`, name `sdk-regression-check`) — not the
`Skill` tool — and run its drift check. If a major version has shifted,
run the smoke test before trusting the patterns.
...
browser extension wallet, load the `midnight-dapp-dev:dapp-connector`
skill via Tome's `get_skill` MCP tool (plugin `midnight-dapp-dev`, name
`dapp-connector`) — not the `Skill` tool — instead. The two skills are complementary: a DApp typically uses the
