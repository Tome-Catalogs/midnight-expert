...
- You do **not** need to set `additionalFeeOverhead` for registration. Setting it
  ...
  compute a zero fee and are rejected as NotNormalized (error **117** —
  invoke the `midnight-status-codes:lookup` command (Tome MCP prompt, not
  the `Skill` tool) with `117`).
