...
1. Build a sender `WalletFacade` from the genesis seed (load the
   `midnight-tooling:devnet` skill via Tome's `get_skill` MCP tool — not
   the `Skill` tool — and see its genesis-seed section for the seed value).
