...
2. **For each drifted package**, invoke the `midnight-tooling:view-release-notes`
   command via its Tome MCP prompt (not the `Skill` tool) scoped to the
   version range from PINNED to LATEST.
