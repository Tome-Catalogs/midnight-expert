...
DUST is the fee-resource produced by locking NIGHT UTXOs on Midnight. Its economics (generation rate, capacity, decay) are defined by the ledger (`midnight-ledger`) and the on-chain DUST parameters, not by the indexer. For the token model and economic rationale, load the `core-concepts:tokenomics` skill via Tome's `get_skill` MCP tool (plugin `core-concepts`, name `tokenomics`) — do not use the `Skill` tool, as this skill lives in a Tome catalog; this file covers only the indexer's storage representation.
...
Purpose: lets API clients determine the current tree upper bound from a single `Block` row without scanning `regular_transactions`. For the GraphQL `@beta` surface that uses these, load the `midnight-indexer:indexer-graphql-api` skill via Tome's `get_skill` MCP tool (plugin `midnight-indexer`, name `indexer-graphql-api`) — do not use the `Skill` tool, as this skill lives in a Tome catalog — then see `references/dust-beta-api.md`.
