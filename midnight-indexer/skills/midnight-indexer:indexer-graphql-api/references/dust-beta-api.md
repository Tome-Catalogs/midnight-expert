...
- For the underlying storage model (how the commitment and generation trees are persisted, nullifier table structure, and DUST/Night relationship), load the `midnight-indexer:indexer-data-model` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) and see `references/dust-and-spo-data.md`.
