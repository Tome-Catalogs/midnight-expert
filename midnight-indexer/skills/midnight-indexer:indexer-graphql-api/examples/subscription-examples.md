...
In the browser, use the `graphql-ws` library with the same protocol; the SDK's `indexerPublicDataProvider` handles this for you (load the `midnight-dapp-dev:midnight-sdk` skill via Tome's `get_skill` MCP tool — not the `Skill` tool — for details).
...
The devnet's DUST backlog begins with a `ParamChange` (the initial DUST parameters, event id `1`) followed by a run of `DustInitialUtxo` events (each NIGHT UTXO registered for DUST generation). For the storage-side model behind these events, load the `midnight-indexer:indexer-data-model` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) and see `references/dust-and-spo-data.md`.
