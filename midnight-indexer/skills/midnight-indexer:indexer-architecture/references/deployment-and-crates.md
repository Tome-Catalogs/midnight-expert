...
Docker profiles in `docker-compose.yaml`: `cloud` services — `chain-indexer`, `wallet-indexer`, `indexer-api`, `spo-indexer`, `postgres`, `nats`; `standalone` services — `indexer-standalone`. The `node` service is in both profiles (`docker-compose.yaml:143–177`). For running the local devnet, load the `midnight-tooling:devnet` skill via Tome's `get_skill` MCP tool (not the `Skill` tool).
