...
> NATS connection events are covered fully in the `midnight-indexer:indexer-architecture` skill (load it via Tome's `get_skill` MCP tool, not `Skill`) → `references/nats-messaging.md`. Summary only:
...
|---|---|---|
| **NATS unreachable at startup** (cloud mode) | `async_nats::ConnectError` wrapped as `"cannot create NATS based publisher/subscriber"` propagated to `"process exited with ERROR"`. Process exits immediately. | Ensure the NATS container is healthy before starting indexer services. In compose, add `depends_on: nats: condition: service_started` (already present in the reference compose). Load `midnight-indexer:indexer-architecture` via Tome's `get_skill` MCP tool (not `Skill`) and see `references/nats-messaging.md`. |
| **Node WebSocket unreachable** | Reconnect back-off begins (10 ms → 10 s). After 30 attempts (≈ 5 min) the subxt client is exhausted and the error propagates to `"process exited with ERROR"`. | Check `APP__INFRA__NODE__URL`. Verify the node container is reachable. Load `midnight-indexer:indexer-architecture` via Tome's `get_skill` MCP tool (not `Skill`) and see `references/configuration-reference.md`. |
| **Missing `APP__INFRA__SECRET`** (wallet-indexer / indexer-api / standalone) | Config deserialization fails; `"load configuration"` context in `"process exited with ERROR"`. | Set `APP__INFRA__SECRET` to a hex-encoded 32+ byte key. Load `midnight-indexer:indexer-architecture` via Tome's `get_skill` MCP tool (not `Skill`) and see `references/configuration-reference.md`. |
