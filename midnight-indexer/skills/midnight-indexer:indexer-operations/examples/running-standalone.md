...
For executed subscription walkthroughs over WebSocket, load the `midnight-indexer:indexer-graphql-api` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) and see `examples/subscription-examples.md`.
