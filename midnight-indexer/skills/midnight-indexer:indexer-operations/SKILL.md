...
Operational guide for running, health-checking, monitoring, and troubleshooting the Midnight indexer. For starting and stopping the local Docker stack, load the `midnight-tooling:devnet` skill via Tome's `get_skill` MCP tool (not the `Skill` tool). For the full observability and troubleshooting reference, see `references/monitoring-and-troubleshooting.md` in this skill.

## Deployment Recap

The indexer ships in two deployment shapes. For full detail, load the `midnight-indexer:indexer-architecture` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) and see `references/deployment-and-crates.md`.
...
Configuration keys live under `APP__` in environment or in `config.yaml`. Load the `midnight-indexer:indexer-architecture` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) and see `references/configuration-reference.md` for the full key listing.
...
Config keys live under `APP__INFRA__NODE__` (the standalone binary also has a separate `APP__INFRA__SPO_NODE__` for its Cardano-side connection). Load the `midnight-indexer:indexer-architecture` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) and see `references/configuration-reference.md`.
...
For the GraphQL mutations (`connect`, `disconnect`) and the `shieldedTransactions` subscription, load the `midnight-indexer:indexer-graphql-api` skill via Tome's `get_skill` MCP tool (not the `Skill` tool).
...
For subjects, consumer groups, and failure modes, load the `midnight-indexer:indexer-architecture` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) and see `references/nats-messaging.md`.
