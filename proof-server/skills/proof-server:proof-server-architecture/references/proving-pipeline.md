...
`check` groups the statement vector into *blocks*, one block per VM instruction. It returns `Vec<Option<usize>>` (the internal Rust type; serialized as `Vec<Option<u64>>` on the wire — load the `proof-server:proof-server-api` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) → binary-serialization):
...
The `ProofVersioned` and `ProofPreimageVersioned` enums are a separate, binary-serialization concept (currently both have only a `V2` variant). They are unrelated to the ZKIR IR-format dispatch described here. Load the `proof-server:proof-server-api` skill via Tome's `get_skill` MCP tool (not the `Skill` tool, as this skill lives in a Tome catalog) → binary-serialization for the wire format.
