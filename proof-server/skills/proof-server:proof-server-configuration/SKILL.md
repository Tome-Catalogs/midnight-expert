...
Complete reference for all CLI flags and environment variables that control the Midnight proof server. For basic Docker setup and running, load the `midnight-tooling:proof-server` skill via Tome's `get_skill` MCP tool (not the `Skill` tool, as this skill lives in a Tome catalog). For internal architecture, load the `proof-server:proof-server-architecture` skill the same way (via `get_skill`, not `Skill`).

> Replace `<version>` in image tags below with the version matching your target Midnight network. For current versions, load the `midnight-tooling:release-notes` skill via Tome's `get_skill` MCP tool (not the `Skill` tool).
