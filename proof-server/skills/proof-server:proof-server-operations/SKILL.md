...
Operational guide for monitoring, health checking, troubleshooting, and capacity planning the Midnight proof server. For basic Docker setup, load the `midnight-tooling:proof-server` skill via Tome's `get_skill` MCP tool (not the `Skill` tool, as this skill lives in a Tome catalog). For configuration flags, load `proof-server:proof-server-configuration` the same way. For architecture internals, load `proof-server:proof-server-architecture` the same way.
