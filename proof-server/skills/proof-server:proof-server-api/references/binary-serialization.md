...
**`/check` response semantics:** the `Vec<Option<u64>>` values carry branch-omission and padding information from the circuit check, **not** a constraint pass/fail result. See the `proof-server:proof-server-architecture` skill (load via Tome's `get_skill` MCP tool, not the `Skill` tool) → `proving-pipeline.md` for interpretation.
...
Clients rarely construct these bytes by hand. The wallet SDK (`httpClientProofProvider`) serializes and deserializes them automatically. If you are writing a custom client or debugging raw traffic, see `examples/constructing-a-prove-request.md` for a worked assembly walkthrough, and load the `proof-server:proof-server-integration` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) for how the SDK invokes the server end-to-end.
