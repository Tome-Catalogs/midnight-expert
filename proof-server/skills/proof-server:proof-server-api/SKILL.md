...
Complete HTTP API reference for the Midnight proof server. The server listens on port 6300 by default. For basic setup and Docker usage, load the `midnight-tooling:proof-server` skill via Tome's `get_skill` MCP tool (not the `Skill` tool). For internal architecture details, load the `proof-server:proof-server-architecture` skill the same way.
...
> Replace `<version>` with the version matching your target Midnight network. Load the `midnight-tooling:release-notes` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) for current versions.
