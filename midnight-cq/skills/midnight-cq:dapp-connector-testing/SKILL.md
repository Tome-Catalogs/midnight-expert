...
Skill names above are bare same-plugin references (e.g. `wallet-testing` is
`midnight-cq:wallet-testing`). Load any of them via Tome's `get_skill` MCP
tool — not the `Skill` tool — since these skills live in a Tome catalog.
...
| Using wallet stub for contract logic testing | Wrong tool; contract logic needs the simulator | Load `midnight-cq:compact-testing` via Tome's `get_skill` (not `Skill`) for contract logic |
