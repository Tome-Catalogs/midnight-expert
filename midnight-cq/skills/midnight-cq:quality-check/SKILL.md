...
> To set up quality tooling from scratch, load `midnight-cq:quality-init` via Tome's `get_skill` (not `Skill`).
...
- Setting up quality tooling from scratch → load `midnight-cq:quality-init` via Tome's `get_skill` (not `Skill`)
- Writing new Compact contract tests → load `midnight-cq:compact-testing` via Tome's `get_skill` (not `Skill`)
- Writing new DApp E2E tests → load `midnight-cq:dapp-testing` via Tome's `get_skill` (not `Skill`)
