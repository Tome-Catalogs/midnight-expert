...
Load any skill named above via Tome's `get_skill` MCP tool — not the `Skill`
tool — since these skills live in a Tome catalog.
...
- Testing Compact contract logic (load `midnight-cq:compact-testing` via Tome's `get_skill`, not `Skill`)
- Testing DApp UI flows end-to-end (load `midnight-cq:dapp-testing` via Tome's `get_skill`, not `Skill`)
- Testing wallet SDK implementations (load `midnight-cq:wallet-testing` via Tome's `get_skill`, not `Skill`)
- Testing DApp Connector API integration (load `midnight-cq:dapp-connector-testing` via Tome's `get_skill`, not `Skill`)
