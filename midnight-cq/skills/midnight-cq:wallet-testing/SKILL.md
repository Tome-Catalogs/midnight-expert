...
- Testing DApp code that integrates via the DApp Connector API (load `midnight-cq:dapp-connector-testing` via Tome's `get_skill`, not `Skill`)
- Testing Compact contracts (load `midnight-cq:compact-testing` via Tome's `get_skill`, not `Skill`)
- Testing DApp UI end-to-end (load `midnight-cq:dapp-testing` via Tome's `get_skill`, not `Skill`)
