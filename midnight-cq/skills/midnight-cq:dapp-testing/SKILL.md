...
> "If you're testing contract logic, load the `midnight-cq:compact-testing`
> skill via Tome's `get_skill` MCP tool (not `Skill`). If you're testing that
> the UI correctly calls contracts and displays results, you're here."
...
| Question | Answer |
|----------|--------|
| Does the test call a Compact circuit directly? | Load `midnight-cq:compact-testing` via Tome's `get_skill` (not `Skill`) |
