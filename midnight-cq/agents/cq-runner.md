...
3. Recommend loading the `midnight-cq:quality-init` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) to set up the missing tooling.
...
> Recommendation: Load the `midnight-cq:quality-init` skill via Tome's `get_skill` MCP tool (not the `Skill` tool) to scaffold Biome, configure Husky hooks, and set up CI workflows.
...
- For setup tasks (adding Biome, configuring Vitest, scaffolding CI), recommend loading `midnight-cq:quality-init` via Tome's `get_skill` MCP tool (not `Skill`) — that skill handles writes.
- For writing new contract tests, recommend loading `midnight-cq:compact-testing` via Tome's `get_skill` MCP tool (not `Skill`).
- For writing new DApp E2E tests, recommend loading `midnight-cq:dapp-testing` via Tome's `get_skill` MCP tool (not `Skill`).
