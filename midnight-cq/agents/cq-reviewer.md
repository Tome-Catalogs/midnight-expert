...
Before scanning anything, load the following skills via Tome's `get_skill` MCP tool (not the `Skill` tool) so you have authoritative standards to measure against:
...
- **No fixes**: you document; you do not implement. If the user asks you to fix something after reading the report, tell them to load the `midnight-cq:quality-init`, `midnight-cq:compact-testing`, or `midnight-cq:dapp-testing` skills via Tome's `get_skill` MCP tool (not the `Skill` tool) for guided remediation.
